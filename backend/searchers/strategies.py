...
class DBSearchingStrategy(SearchingStrategy):
    def __init__(self, db: DndDatabase) -> None:
        self.__db = db

    def __get_relevant_elements(self, query: str) -> List[Dict[str, Any]]:
        found = set()
        results = []
        query = query.strip()
        db_query = {"name": {"$regex": f".+?{query}.+?", "$options": "i"}}
        for collection_name in self.__db.list_collection_names():
            data = self.__db.find(collection_name, db_query)
            names = {item["name"] for item in data}
            newly_found = names - found
            found |= names
            result = self.__db.find(
                collection_name, {"name": {"$in": list(newly_found)}}
            )
    ...
    def search(self, query: str, n: int = -1) -> List[Dict[str, Any]]:
        ...
        elements = self.__get_relevant_elements(query)
        names = [element["name"] for element in elements]
        # getting the closest matches to the query
        accuracy = 0.1
        tries = int(1.0 / accuracy) + 1
        for i in range(tries):
            cutoff = 1.0 - (accuracy * i)
            closest_matches = difflib.get_close_matches(
                query, names, cutoff=cutoff, n=n
            )
            if closest_matches:
                break

        matches = [
            element for element in elements if element["name"] in closest_matches
        ]
        return matches
